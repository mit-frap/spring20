(** * 6.822 Formal Reasoning About Programs, Spring 2020 - Pset 11 *)

Require Import Frap.
Require Export Eqdep. (* for inj_pair2 as referenced in a hint *)

(* Authors: Peng Wang (wangpeng@csail.mit.edu),
...
Module Type S.
  Axiom ht_increment : forall init, (* 25% *)
    hoare_triple
      (fun h => h $! 0 = init)
      (fun _ _ => False)
  ...
  (* 75%, with partial credit for each lemma in the hints *)
  Axiom hoare_triple_sound : forall (t : Set) P (c : cmd t) Q,
